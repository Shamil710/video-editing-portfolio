...
/* ══════════════════════════════════════════════════════════════════
   LOADER SCREEN
   Concept: Anamorphic aperture — two letterbox bars slide in from
   top/bottom, the name assembles letter-by-letter, a single gold
   line fills as the progress bar, then the bars slide OUT revealing
   the site. Clean. Cinematic. Zero clutter.
   ══════════════════════════════════════════════════════════════════ */

const NAME_CHARS = ["R", "O", "B", "I", "N", " ", "D"];

export function LoaderScreen() {
  const FILL_DURATION = 1800; // ms for progress line to fill

  const [phase, setPhase] = useState<"in" | "fill" | "out">("in");
  const [progress, setProgress] = useState(0);
  const progressMV = useMotionValue(0);
  const lineWidth = useTransform(progressMV, [0, 100], ["0%", "100%"]);

  /* ── Phase sequencing ─────────────────────────────────────────── */
  useEffect(() => {
    // bars slide in (400ms) → hold briefly → start fill
  ...
  useEffect(() => {
    ...
    const tick = (now: number) => {
      const pct = Math.min(1, (now - start) / FILL_DURATION);
  ...
  return (
    <motion.div
      ...
      transition={{
        duration: 0.55,
        delay: phase === "out" ? 0.38 : 0,
        ease: [0.22, 1, 0.36, 1],
      }}
      aria-label="Loading Robin D portfolio"
    >
      {/* ── Subtle ambient center bloom ─────────────────────────── */}
      <div
        className="pointer-events-none absolute inset-0"
        style={{
          background:
            "radial-gradient(ellipse 55% 38% at 50% 50%, rgba(212,175,55,0.04) 0%, transparent 100%)",
        }}
      />
      ...
      <motion.div
        ...
        style={{ background: "#060606", zIndex: 2 }}
        initial={{ height: "0%" }}
        animate={{
          height: barsVisible ? "22%" : "0%",
        }}
        transition={{
          duration: barsVisible ? 0.52 : 0.48,
          ease: barsVisible ? [0.22, 1, 0.36, 1] : [0.76, 0, 0.24, 1],
        }}
      >
        {/* top inner edge glow */}
        <div
          ...
          style={{
            background:
              "linear-gradient(90deg, transparent 0%, rgba(212,175,55,0.18) 30%, rgba(212,175,55,0.35) 50%, rgba(212,175,55,0.18) 70%, transparent 100%)",
          }}
        />
      </motion.div>
      ...
      <div
        ...
        style={{ gap: "clamp(1.2rem, 3vh, 2rem)" }}
      >
        {/* Role eyebrow */}
        <motion.p
          className="tracking-[0.5em] uppercase text-white/28"
          style={{
            fontFamily: "'Syne', sans-serif",
        ...
        >
          Motion Editor &amp; Content Creator
        </motion.p>

        {/* ── NAME — letter by letter stagger ─────────────────────── */}
        <div
          className="flex items-baseline"
          style={{ gap: "clamp(0.04em, 0.3vw, 0.08em)" }}
        >
          {NAME_CHARS.map((ch, i) => {
            const isD = ch === "D";
            const isSpace = ch === " ";
        ...
        <motion.div
          ...
          initial={{ opacity: 0 }}
          animate={{ opacity: phase === "fill" || phase === "out" ? 1 : 0 }}
        ...
        >
          <motion.div
            ...
            style={{
              width: lineWidth,
              background:
                "linear-gradient(90deg, rgba(143,106,31,0.7) 0%, rgba(242,220,138,1) 50%, rgba(255,241,191,1) 100%)",
              boxShadow:
          ...
          <motion.div
            ...
            style={{
              x: useTransform(progressMV, (v) => `calc(${v}% - 2.5px)`),
              background: "rgba(255,241,191,1)",
              boxShadow: "0 0 8px rgba(255,241,191,0.9)",
            }}
          />
        </motion.div>
        ...
        <motion.div
          ...
          initial={{ opacity: 0 }}
          animate={{ opacity: phase === "fill" || phase === "out" ? 0.35 : 0 }}
          transition={{ duration: 0.4, delay: 0.15 }}
          style={{
            fontFamily: "'DM Sans', 'Plus Jakarta Sans', sans-serif",
          }}
        >
          <span className="text-[0.55rem] tracking-[0.32em] uppercase text-white/50">
            Portfolio
          </span>
